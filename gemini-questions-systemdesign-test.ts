...
// Calls the REAL, unmodified generateQuestionsForCategory() for the "system-design"
// category with a "spy" LLMCaller capturing the full StepResult. No production code modified.
...
// Same technical-kind requirements as the earlier Technical test — matches production
// exactly: orchestrator.ts feeds the "system-design" category the same grouped.technical
// set it feeds "technical", not a separate grouping.
...
async function main() {
  ...
  console.log("Calling the real generateQuestionsForCategory('system-design') with an instrumented LLMCaller...\n");

  const start = Date.now();
  const questions = await generateQuestionsForCategory(
    { category: "system-design", requirements: REQUIREMENTS, roleContext: ROLE_CONTEXT },
    spyCall,
    1
  );
...
main().catch((err) => {
  console.error("System-design question generation test threw an unexpected error:", err);
  process.exitCode = 1;
});
